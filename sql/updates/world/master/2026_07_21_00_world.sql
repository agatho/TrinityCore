...
--
-- Folded from sql/custom/battlepay/battlepay_product.sql. The TC updater only
-- applies sql/updates/**, so schema parked under sql/custom is never installed:
-- BattlePayMgr::LoadProducts() then aborted startup right after
-- "Loading in-game Shop (BattlePay) catalog..." with
--   [1146] Table 'world.battlepay_product' doesn't exist
-- (the loader's "empty or missing" message never gets a chance to run, because
-- the database layer treats the missing table as fatal first).
--
-- Each row maps a catalog productID (advertised to the client in
-- GET_PRODUCT_LIST_RESPONSE) to how it is paid for and what it grants.
...
-- The productIds below match the reskinned demo catalog produced by
-- gen_shop_catalog.py; regenerate both together if the product set changes.
-- ============================================================================
