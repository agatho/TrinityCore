...
// creature entries ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬ï¿½ these are separate from type 8 critters.
...
float Creature::GetBaseDamageForLevel(uint8 level) const
{
    CreatureTemplate const* cInfo = GetCreatureTemplate();
    CreatureDifficulty const* creatureDifficulty = GetCreatureDifficulty();
    float baseDamage = sDB2Manager.EvaluateExpectedStat(ExpectedStatType::CreatureAutoAttackDps, level, creatureDifficulty->GetHealthScalingExpansion(), m_unitData->ContentTuningID, Classes(cInfo->unit_class), 0);
