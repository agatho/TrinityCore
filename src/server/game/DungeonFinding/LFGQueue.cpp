...
LfgCompatibility LFGQueue::CheckCompatibility(GuidList check)
{
    ...
    // a group smaller than the dungeons need is always compatible (one-player scenarios are complete on their own)
    uint8 groupSize = LFGMgr::GetGroupSizeForDungeons(QueueDataStore[check.front()].dungeons);
