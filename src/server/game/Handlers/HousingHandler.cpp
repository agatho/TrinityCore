...
void WorldSession::HandleHousingSvcsNeighborhoodReservePlot(WorldPackets::Housing::HousingSvcsNeighborhoodReservePlot const& housingSvcsNeighborhoodReservePlot)
{
    ...
    // Route to MoveHouse when the player already owns a house in this neighborhood.
    // The House Finder's "Reserve" button is the same UI in retail whether you're
    // buying a first house or moving an existing one; the server is responsible for
    // routing based on the player's housing state.
    Housing* existingHousing = player->GetHousingForNeighborhood(neighborhood->GetGuid());
    if (existingHousing)
    {
        ...
        {
            TC_LOG_INFO("housing",
                "ReservePlot: target plot {} is the player's current plot; treating as no-op success",
                plotIndex);
            WorldPackets::Housing::HousingSvcsNeighborhoodReservePlotResponse response;
        ...
        HousingResult moveResult = neighborhood->MoveHouse(player->GetGUID(), plotIndex);
        if (moveResult == HOUSING_RESULT_SUCCESS)
        {
            existingHousing->SetPlotIndex(plotIndex);
            existingHousing->SyncUpdateFields();
            ...
            if (HousingMap* housingMap = dynamic_cast<HousingMap*>(player->GetMap()))
            {
                ...
                auto fixtureOverrides = existingHousing->GetFixtureOverrideMap();
                housingMap->SpawnHouseForPlot(plotIndex, nullptr,
                    static_cast<int32>(existingHousing->GetCoreExteriorComponentID()),
                    static_cast<int32>(existingHousing->GetHouseType()),
                    fixtureOverrides.empty() ? nullptr : &fixtureOverrides);
            }

            // Refresh map pins for everyone in the neighborhood
            neighborhood->RefreshMirrorDataForOnlineMembers();
        }

        WorldPackets::Housing::HousingSvcsNeighborhoodReservePlotResponse response;
        response.Result = static_cast<uint8>(moveResult);
        SendPacket(response.Write());

        TC_LOG_INFO("housing",
            "CMSG_HOUSING_SVCS_NEIGHBORHOOD_RESERVE_PLOT routed as MoveHouse: from plot {} to plot {} (Result {})",
            oldPlotIndex, plotIndex, uint32(moveResult));
        return;
    }
