...
#define BnetGameAccountInfo "a.id AS account_id, a.username, ab.bandate, ab.unbandate AS account_unbandate, ab.unbandate = ab.bandate AS is_banned, aa.SecurityLevel"
    ...
    PrepareStatement(LOGIN_SEL_ACCOUNT_PERKS_TENDER, "SELECT amount FROM battlenet_account_perks_tender WHERE accountId = ?", CONNECTION_ASYNC);
    PrepareStatement(LOGIN_REP_ACCOUNT_PERKS_TENDER, "REPLACE INTO battlenet_account_perks_tender (accountId, amount) VALUES (?, ?)", CONNECTION_ASYNC);
