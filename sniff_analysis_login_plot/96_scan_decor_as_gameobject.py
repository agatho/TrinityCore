#!/usr/bin/env python3
"""Scan retail sniffs for GameObject CREATEs that carry FHousingDecor_C fragment.

These are real GameObjects (chairs, chests, etc.) spawned AS decor — they retain
normal GO functionality (chair=sit, chest=open, etc.) while being registered as
decor via FHousingDecor_C.

Contrast with MeshObject decor (visual-only mesh) and Housing/sub1 entities.
"""
import struct
import os
import glob
...
def scan(body):
    """Find GameObject (HighGuid=11) CREATEs that contain FHousingDecor_C in
    their fragment list.
    """
    results = []
    if len(body) < 10:
    ...
    while i < len(body) - 10:
        ...
        if lo is None:
            i += 1; continue
        high = (hi >> 58) & 0x3F
        if high != HIGH_GUID_GAMEOBJECT:
            i += 1; continue
        if next_p >= len(body):
            i += 1; continue
        obj_type = body[next_p]
        if obj_type != 8:  # TYPEID_GAMEOBJECT
            i += 1; continue
        ...
        if anchor_dist > 256:
            i += 1; continue
        # Must contain FHousingDecor_C (20)
        if 20 not in frags:
            i = bend; continue
        results.append({
            'pos': i, 'lo': lo, 'hi': hi, 'obj_type': obj_type,
            'fragments': frags, 'fsize': fsize, 'anchor_dist': anchor_dist,
            'block_end': bend,
        })
        i = bend
    return results
...
def main():
    ...
    print(f'Scanning {len(sniffs)} retail sniffs for GameObject CREATEs carrying FHousingDecor_C\n')
    ...
    for pkt in sorted(sniffs):
        ...
        for idx, d, op, body in iter_packets(pkt):
            ...
            for r in scan(body):
                count += 1
                total += 1
                sig = tuple(r['fragments'])
                per_sig[sig] = per_sig.get(sig, 0) + 1
                sniff_sigs[sig] = sniff_sigs.get(sig, 0) + 1
        if count > 0:
            per_sniff.append((name, count, sniff_sigs))

    print('\n=== Per-sniff counts ===\n')
    for name, count, sigs in per_sniff:
        print(f'  {name}: {count} GameObject-with-FHousingDecor_C CREATEs')
        for sig, c in sorted(sigs.items(), key=lambda x: -x[1]):
            frags = ','.join(FRAG_NAMES.get(f, str(f)) for f in sig)
            print(f'    {c:3d}× [{frags}]')

    print(f'\n=== Cross-sniff signature totals (total={total}) ===\n')
    for sig, count in sorted(per_sig.items(), key=lambda x: -x[1]):
        frags = ','.join(FRAG_NAMES.get(f, str(f)) for f in sig)
        print(f'  {count:4d}× [{frags}]')

    print(f'\nTOTAL GameObject decor CREATEs: {total} across {len(per_sniff)}/{len(sniffs)} sniffs')
